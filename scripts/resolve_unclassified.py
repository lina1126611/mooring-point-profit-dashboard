...
#
# 2026-08-03 씨웨이테크 실데이터 수령 시점에 비웠다. 여기 있던 항목은
# 전부 generate_sample_data.py 가 만든 가상 거래에 대한 판단이라, 실제
# 회사 원장에는 대상이 0건이다. 남겨 두면 "적용됐나?" 를 매번 되묻게
# 되고, 더 나쁘게는 샘플용 판단을 실제 회사의 확정 판단으로 오해하게 된다.
#
# 실제 미분류를 눈으로 보고 대표·경리와 합의한 것만 여기에 다시 쌓는다.
#
# [보존] 샘플 전용이었던 판단 — data/sample/ 로 데모를 돌릴 때만 유효하다.
#   ("정산 차액", "사무관리비", "고정", "본사 공통·매월 반복 → 고정")
#   근거: 18건 전부 project_id 가 비어 있는 본사 공통 지출이고 매달 반복된다.
#   수주가 없어도 나가므로 고정. rules.py 의 '기타 운영비 → 사무관리비(고정)'
#   와 같은 논리다. 변동으로 두면 현장에 안 붙어 있어 변동비 합산(project_id
#   로 묶음)에서 빠지고, 미분류와 똑같이 원가에서 사라진다.
#   ※ 되살리면 샘플 데모의 미분류 18건이 다시 정리된다.
# ---------------------------------------------------------------
RESOLUTIONS = []
